...
type ArtistCardProps = {
  ...
  onClick: (artist: ArtistContent) => void
}
    ...
    <button type="button" className="block w-full min-w-0 text-left" onClick={() => onClick(artist)} aria-label={`Open ${artist.name} details`}>
      <Card className="h-full overflow-hidden">
        <div className="rounded-xl border border-black/10 bg-base/40 p-2">
          <img src={assetPath(artist.profileImage)} alt={artist.name} className="h-60 w-full rounded-lg object-contain object-center sm:h-64" loading="lazy" />
        </div>
        <div className="mt-4">
